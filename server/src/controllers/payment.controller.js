...
const PaymentController = {
    getPayments: async (req, res) => {
        ...
        if (req.user.role === "Admin") {
            payments = await PaymentModel.getAllPayments();
        } else {
            payments = await PaymentModel.getPaymentsByUserId(req.user.userId);
        }
...
handlePayOSWebhook: async (req, res) => {
    ...
    getPaymentStats: async (req, res) => {
        console.log("Đã gọi getPaymentStats - User:", req.user);
        try {
            if (req.user.role !== "Admin") {
            console.log("User không phải Admin, từ chối truy cập");
            ...
            console.log("Dữ liệu thống kê:", stats);
            return res.status(200).json({ data: stats });
        } catch (error) {
            console.error("Lỗi trong getPaymentStats:", error.message);
            return res.status(500).json({ message: error.message });
        }
    },
};
